...
public class VanillaStateTest
{
    ...
    private static final VanillaThro _0 = get(0);
    ...
    public void getAvailableThrows() throws Exception
    {
        ...
        softly.assertThat(availableThrows.size()).isEqualTo(27);
        softly.assertThat(availableThrows.contains(_0)).isFalse();
        softly.assertThat(availableThrows.contains(_1)).isFalse();
        softly.assertThat(availableThrows.contains(_2)).isFalse();
        ...
        softly.assertThat(availableThrows1.contains(_1)).isFalse();
        softly.assertThat(availableThrows1.contains(_2)).isFalse();
        softly.assertThat(availableThrows1.contains(_3)).isFalse();
        softly.assertThat(availableThrows1.contains(_4)).isFalse();
        softly.assertThat(availableThrows1.contains(_5)).isFalse();
    }
    ...
    public void automatedTestGetThro() throws Exception
    {
        VanillaStateGenerator.getAllStates(5, 8)
            .forEach(state ->
            {
                for (final Thro thro : state.getAvailableThrows())
                {
                    final State descendant = state.thro(thro);
                    softly.assertThat(state.getThrow(descendant)).isEqualTo(thro);
                }
            });
    }
    ...
    public void getNextStates() throws Exception
    {
        ...
        softly.assertThat(nextStates).hasSize(27);
        softly.assertThat(nextStates).contains(XXX__);
        softly.assertThat(nextStates).contains(XX_X_);
        softly.assertThat(nextStates).contains(XX__X);

        final Set<State> nextStates1 = _XXX_.getNextStates();
        softly.assertThat(nextStates1).hasSize(1);
        softly.assertThat(nextStates1).contains(XXX__);

        final Set<State> nextStates2 = _XXX__.getNextStates();
        softly.assertThat(nextStates2).hasSize(1);
        softly.assertThat(nextStates2).contains(XXX___);

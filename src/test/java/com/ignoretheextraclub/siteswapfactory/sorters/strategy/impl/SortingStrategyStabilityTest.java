...
import com.ignoretheextraclub.siteswapfactory.generators.FourHandedSiteswapGenerator;
import com.ignoretheextraclub.siteswapfactory.generators.TwoHandedSiteswapGenerator;
 ...
 */
public class SortingStrategyStabilityTest
{
    private static final Logger LOG = LoggerFactory.getLogger(SortingStrategyStabilityTest.class);
        ...
        final long count = FourHandedSiteswapGenerator.all(numObjects, maxThrow).generate()
                ...
                .peek(siteswap ->
                {
                    ...
                    for (final String constructor : SortingTestUtils.getRotations(strSiteswap))
                    {
                        try
                        {
                            final FourHandedSiteswap fhs = SiteswapFactory.createFHS(constructor);
        ...
        final long count = TwoHandedSiteswapGenerator.all(numObjects, maxThrow).generate()
                ...
                .peek(siteswap ->
                {
                    ...
                    for (final String constructor : SortingTestUtils.getRotations(strSiteswap))
                    {
                        try
                        {
                            final TwoHandedVanillaSiteswap ths = SiteswapFactory.createTHS(constructor);
